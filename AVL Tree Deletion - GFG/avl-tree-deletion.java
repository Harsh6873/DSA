...
class Sol
{
    public static Node deleteNode(Node root, int key)
    {
        ...
        treeInorder(tree, root, key);
        return build(tree, 0, tree.size() - 1);
    }
    public static void treeInorder(ArrayList<Integer> ls, Node currentNode, int key) {
        if (currentNode != null) {
            treeInorder(ls, currentNode.left, key);
            if (currentNode.data != key) {
                ls.add(currentNode.data);
            }
            treeInorder(ls, currentNode.right, key);
        }
    }

    public static Node build(ArrayList<Integer> ar, int startIdx, int endIdx) {
        if (startIdx > endIdx) return null;
        int mid = (startIdx + endIdx) / 2;
        Node rootNode = new Node(ar.get(mid));
        rootNode.left = build(ar, startIdx, mid - 1);
        rootNode.right = build(ar, mid + 1, endIdx);
        return rootNode;
    }
}